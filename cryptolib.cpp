...
std::string static toHex(const std::string& data)
{
    std::stringstream ss;
...
cpp_int static modInverse(cpp_int a, cpp_int m)
{
    cpp_int m0 = m, t, q;
    cpp_int x0 = 0, x1 = 1;
...
RSAEncryptor::RSAEncryptor(std::string prime1, std::string prime2)
{
    cpp_int p_val(prime1);
    cpp_int q_val(prime2);

    cpp_int cpp_n = p_val * q_val;
    this->n = cpp_n.str();

    cpp_int phi = (p_val - 1) * (q_val - 1);
    cpp_int cpp_e = 65537;

    while (boost::integer::gcd(cpp_e, phi) != 1) 
    {
        cpp_e += 2;
    }

    cpp_int cpp_d = modInverse(cpp_e, phi);
...
std::string RSAEncryptor::encrypt(std::string message, std::string publicKeyN, std::string publicKeyE)
{
    try 
    {
        cpp_int m = 0;
        ...
        cpp_int n_val(publicKeyN);
        cpp_int e_val(publicKeyE);

        if (m >= n_val)
        {
            return "ERROR_MSG_TOO_LARGE";
        }
        cpp_int c = powm(m, e_val, n_val);
    ...
    {
        std::cerr << "Eroare la criptare: " << e.what() << std::endl;
        return "";
    }
}
std::string RSAEncryptor::decrypt(std::string cipher)
{
    try
    {
        cpp_int c("0x" + cipher);
        cpp_int n_val(this->n);
        cpp_int d_val(this->d);

        cpp_int m = powm(c, d_val, n_val);
        ...
        while (temp_m > 0)
        {
            unsigned char ch = static_cast<unsigned char>(temp_m & 0xFF);
            text = (char)ch + text;
            temp_m >>= 8;
        }
        return text;
    ...
    {
        std::cerr << "Eroare la decryptare: " << e.what() << std::endl;
        return "";
    }
}

std::string RSAEncryptor::getN() { return this->n; }
std::string RSAEncryptor::getE() { return this->e; }
std::string RSAEncryptor::getD() { return this->d; }


Blowfish::Blowfish(std::string rawKey) 
{
    if (rawKey.length() > 56) 
    {
        this->key = rawKey.substr(0, 56);
    }
    else if (rawKey.length() < 4) 
    {
        srand(static_cast<unsigned int>(time(0)));
        while (rawKey.length() < 4) 
        {
            rawKey += static_cast<char>(rand() % 94 + 32);
        }
        this->key = rawKey;
    }
    else 
...
uint32_t Blowfish::encryptBlock(uint32_t block)
{
...
uint32_t Blowfish::decryptBlock(uint32_t block)
{
    uint32_t L = static_cast<uint32_t>(block >> 32);
    uint32_t R = static_cast<uint32_t>(block & 0xFFFFFFFF);
    decryptBlock(L, R);
...
std::string Blowfish::encryptMessage(std::string message) 
{
    while (message.length() % 8 != 0) 
        message += '\0';

    std::string resultBin = "";
    resultBin.reserve(message.length());

    for (size_t i = 0; i < message.length(); i += 8) 
    {
        uint32_t L = ((uint32_t)(unsigned char)message[i] << 24) | ((uint32_t)(unsigned char)message[i + 1] << 16) |
            ((uint32_t)(unsigned char)message[i + 2] << 8) | ((uint32_t)(unsigned char)message[i + 3]);

        uint32_t R = ((uint32_t)(unsigned char)message[i + 4] << 24) | ((uint32_t)(unsigned char)message[i + 5] << 16) |
            ((uint32_t)(unsigned char)message[i + 6] << 8) | ((uint32_t)(unsigned char)message[i + 7]);
        ...
        resultBin.append(buf, 8);
    }

    return toHex(resultBin);
}
std::string Blowfish::decryptMessage(std::string cipherHex) 
{
    if (cipherHex.length() % 16 != 0) 
        return "";

    std::string binaryData = "";
    binaryData.reserve(cipherHex.length() / 2);
    ...
    decryptedText.reserve(binaryData.length());

    for (size_t i = 0; i < binaryData.length(); i += 8) 
    {
        uint32_t L = ((uint32_t)(unsigned char)binaryData[i] << 24) | ((uint32_t)(unsigned char)binaryData[i + 1] << 16) |
            ((uint32_t)(unsigned char)binaryData[i + 2] << 8) | ((uint32_t)(unsigned char)binaryData[i + 3]);

        uint32_t R = ((uint32_t)(unsigned char)binaryData[i + 4] << 24) | ((uint32_t)(unsigned char)binaryData[i + 5] << 16) |
            ((uint32_t)(unsigned char)binaryData[i + 6] << 8) | ((uint32_t)(unsigned char)binaryData[i + 7]);
        ...
        decryptedText.append(buf, 8);
    }

    size_t lastChar = decryptedText.find_last_not_of('\0');
    if (lastChar != std::string::npos) 
    {
        decryptedText.erase(lastChar + 1);
    }
    else 
    {
        decryptedText.clear();
...
std::string Blowfish::getKey() { return this->key; }